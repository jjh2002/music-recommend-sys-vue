  ...
  created() {
    ...
    this.$store.commit('setAutoplay', false);
     if(this.$store.state.music) {
      ...
      this.$axios.get('/api/recommendation', {
        params: {
        ...
        this.cal = response.data.cal

      })
      ;  // 输出 category 参数的值
    if (this.cal===1)
      console.log(this.cal,'hahaha')
      this.$axios.get('/api/cal', {
        params: {
          id: this.$store.state.loginInfo.id
        }
      }).then(response => {
